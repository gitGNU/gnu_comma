...
{
    decl = decl ? decl : cast<EnumerationDecl>(base->getDefiningDecl());
...
{
    IntegerDecl *res;
    res = new IntegerDecl(*this, name, loc, modulus, parent);
    decls.push_back(res);
    return res;
}
...
{
    IntegerType *res;
    decl = decl ? decl : cast<IntegerDecl>(base->getDefiningDecl());
